...
import { handleDeleteJobRole } from "@/lib/actions/admin/jobRole-action";
...
export default function JobTable({
    ...
    const setQuery = (next: Record<string, string | number>) => {
        ...
        router.push(`/admin/jobRole?${q.toString()}`);
    };
    ...
    const onDelete = () => {
        ...
        startTransition(async () => {
            const result = await handleDeleteJobRole(target._id);
            if (result.success) {
                toast.success("Job role deleted");
                setTarget(null);
                router.refresh();
            } else {
                toast.error(result.message || "Failed to delete job role");
            }
        });
    };
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-6 flex items-start justify-center">
            <div className="w-full max-w-5xl bg-white rounded-2xl shadow-md p-8">
                            ...
                            {data?.length ? (
                                data.map((j) => (
                                    <tr key={j._id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-5 py-3.5 text-gray-500">{j.category}</td>
                                        ...
                                        <td className="px-5 py-3.5">
                                            <div className="flex justify-end gap-4 text-xs font-bold uppercase tracking-widest">
                                                <Link href={`/admin/jobRole/${j._id}/edit`} className="text-gray-400 hover:text-gray-900 transition-colors">
                                                    Edit
                                                </Link>
                                                <button
                                                    onClick={() => setTarget(j)}
                ...
                <Modal open={!!target} onClose={() => setTarget(null)} title="Delete Job Role">
