...
import { handleDeleteCourse } from "@/lib/actions/admin/course-action";
...
export default function CourseTable({
    ...
    const setQuery = (next: Record<string, string | number>) => {
        ...
        router.push(`/admin/course?${q.toString()}`);
    };
    ...
    const onDelete = () => {
        ...
        startTransition(async () => {
            const result = await handleDeleteCourse(target._id);
            if (result.success) {
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-6 flex items-start justify-center">
            <div className="w-full max-w-5xl bg-white rounded-2xl shadow-md p-8">
                    ...
                    <table className="w-full text-left text-sm">
                        <thead className="border-b border-gray-100 bg-gray-50 text-xs font-semibold uppercase tracking-widest text-gray-400">
                            <tr>
                                ...
                                <th className="px-5 py-3 font-semibold">Semesters</th>
                        ...
                        <tbody>
                            {data?.length ? (
                                data.map((c) => (
                                    <tr key={c._id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                                        <td className="px-5 py-3.5 font-medium text-gray-900">{c.name}</td>
                                        <td className="px-5 py-3.5 text-gray-500">{c.degree}</td>
                                        <td className="px-5 py-3.5 text-gray-500">{c.durationInSemesters}</td>
                                        ...
                                        <td className="px-5 py-3.5">
                                            <div className="flex justify-end gap-4 text-xs font-bold uppercase tracking-widest">
                                                <Link href={`/admin/course/${c._id}/edit`} className="text-gray-400 hover:text-gray-900 transition-colors">
                                                    Edit
                                                </Link>
                                                <button
                                                    onClick={() => setTarget(c)}
                ...
                <Modal open={!!target} onClose={() => setTarget(null)} title="Delete Course">
