...
import { handleDeleteUniversity } from "@/lib/actions/admin/university-action";
...
export default function UniversityTable({
    ...
    const setQuery = (next: Record<string, string | number>) => {
        ...
        router.push(`/admin/university?${q.toString()}`);
    };
    ...
    const onDelete = () => {
        ...
        startTransition(async () => {
            const result = await handleDeleteUniversity(target._id);
            if (result.success) {
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-6 flex items-start justify-center">
            <div className="w-full max-w-5xl bg-white rounded-2xl shadow-md p-8">
                    ...
                    <table className="w-full text-left text-sm">
                        <thead className="border-b border-gray-100 bg-gray-50 text-xs font-semibold uppercase tracking-widest text-gray-400">
                            <tr>
                                ...
                                <th className="px-5 py-3 font-semibold">Short Name</th>
                            ...
                            {data?.length ? (
                                data.map((u) => (
                                    <tr key={u._id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-5 py-3.5 text-gray-500">{u.shortName}</td>
                                        ...
                                        <td className="px-5 py-3.5">
                                            <div className="flex justify-end gap-4 text-xs font-bold uppercase tracking-widest">
                                                <Link href={`/admin/university/${u._id}/edit`} className="text-gray-400 hover:text-gray-900 transition-colors">
                                                    Edit
                                                </Link>
                                                <button
                                                    onClick={() => setTarget(u)}
                ...
                <Modal open={!!target} onClose={() => setTarget(null)} title="Delete University">
