...
import { handleDeleteOpportunity, handleScrapeOpportunities } from "@/lib/actions/admin/opportunity-action";
...
export default function OpportunityTable({
    ...
    const setQuery = (next: Record<string, string | number>) => {
        ...
        router.push(`/admin/opportunities?${q.toString()}`);
    };
    ...
    const onDelete = () => {
        if (!target) return;
        startDelete(async () => {
            const result = await handleDeleteOpportunity(target._id);
            if (result.success) {
                toast.success("Opportunity deleted");
                setTarget(null);
                router.refresh();
            } else {
                toast.error(result.message || "Failed to delete opportunity");
            }
        });
    };
    ...
    const onScrape = () => {
        setScrapeStats(null);
        startScrape(async () => {
            const result = await handleScrapeOpportunities();
            if (result.success) {
                toast.success("Opportunities refreshed");
                setScrapeStats(result.data ?? null);
                router.refresh();
            } else {
                toast.error(result.message || "Failed to scrape opportunities");
            }
        });
    };
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-6 flex items-start justify-center">
            <div className="w-full max-w-6xl bg-white rounded-2xl shadow-md p-8">
                <div className="mb-6 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
                    ...
                    <div className="flex gap-3">
                        <Link
                            ...
                            className="inline-flex items-center rounded-lg border border-gray-200 px-5 py-2.5 text-xs font-bold uppercase tracking-widest text-gray-700 transition-colors hover:border-gray-400 hover:text-gray-900"
                        >
                            Create New
                        </Link>
                        <button
                        ...
                        >
                            {isScraping ? "Scraping..." : "Scrape Opportunities"}
                        </button>
                    </div>
                </div>
                    ...
                    <div className="mb-6 space-y-2 rounded-xl border border-gray-100 p-4">
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-2 rounded-lg bg-gray-50 px-4 py-2.5 text-sm">
                            <span className="font-medium text-gray-900">
                                {scrapeStats.totalScraped} opportunities from {scrapeStats.sourcesSucceeded.join(", ") || "no sources"}
                            </span>
                            ...
                            {data?.length ? (
                                data.map((o) => (
                                    <tr key={o._id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-5 py-3.5 text-gray-500">{o.organizer}</td>
                                        <td className="px-5 py-3.5 text-gray-500">{o.category ?? "—"}</td>
                                        <td className="px-5 py-3.5 text-gray-500">{o.location}</td>
                                        ...
                                        <td className="px-5 py-3.5">
                                            <div className="flex justify-end gap-4 text-xs font-bold uppercase tracking-widest">
                                                <Link href={`/admin/opportunities/${o._id}/edit`} className="text-gray-400 hover:text-gray-900 transition-colors">
                                                    Edit
                                                </Link>
                                                <button
                                                    onClick={() => setTarget(o)}
                                ...
                                <tr>
                                    <td colSpan={6} className="px-5 py-14 text-center text-gray-400 text-sm">
                                        No opportunities found. Try scraping or creating one to populate this list.
                                    </td>
                                </tr>
                            )}
                        </tbody>
                ...
                <Modal open={!!target} onClose={() => setTarget(null)} title="Delete Opportunity">
