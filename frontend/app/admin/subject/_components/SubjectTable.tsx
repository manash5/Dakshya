...
import { handleDeleteSubject } from "@/lib/actions/admin/subject-action";
...
export default function SubjectTable({
    ...
    const setQuery = (next: Record<string, string | number>) => {
        ...
        router.push(`/admin/subject?${q.toString()}`);
    };
    ...
    const onDelete = () => {
        ...
        startTransition(async () => {
            const result = await handleDeleteSubject(target._id);
            if (result.success) {
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-6 flex items-start justify-center">
            <div className="w-full max-w-5xl bg-white rounded-2xl shadow-md p-8">
                ...
                <div className="overflow-x-auto rounded-xl border border-gray-100">
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody>
                            {data?.length ? (
                                data.map((s) => (
                                    <tr key={s._id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                                        <td className="px-5 py-3.5 font-medium text-gray-900">{s.code}</td>
                                        <td className="px-5 py-3.5 text-gray-500">{s.name}</td>
                                        <td className="px-5 py-3.5 text-gray-500">{s.semester}</td>
                                        <td className="px-5 py-3.5 text-gray-500">{s.credits}</td>
                                        <td className="px-5 py-3.5">
                                            <div className="flex justify-end gap-4 text-xs font-bold uppercase tracking-widest">
                                                <Link href={`/admin/subject/${s._id}/edit`} className="text-gray-400 hover:text-gray-900 transition-colors">
                                                    Edit
                                                </Link>
                                                <button
                                                    onClick={() => setTarget(s)}
                ...
                <Modal open={!!target} onClose={() => setTarget(null)} title="Delete Subject">
